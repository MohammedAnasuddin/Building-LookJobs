...
(async () => {
  ...
  await page.type("#password", "jobsfetch@linkedIN", { delay: 100 })
  ...
  await page.click("li.global-nav__primary-item:nth-of-type(3)", { waitUntil: ['domcontentloaded', 'networkidle2'] });
  ...
  // Ask user for job title and location using prompt
  let job_requested = prompt("Enter the Job Role: ");
  let job_requested_location = prompt("Enter the Preferred Location: ");

  // Select correct input fields dynamically
  ...
  // Wait before proceeding (optional)
  await LinkedIN_Job_Input.press("Enter");

  await page.waitForSelector(".job-card-container"); // Ensure job results have loaded

  // ✅ Capture updated URL with job results
  const searchResultsURL = page.url();

  // await LinkedIN_goto_URL(searchResultsURL)
...
async function LinkedIN_goto_URL(jobURL) {
  const browser = await puppeteer.launch({ defaultViewport: null, headless: false });
  const page = await browser.newPage();
  page.goto(jobURL)
}
